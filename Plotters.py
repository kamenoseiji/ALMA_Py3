...
def plotStokesSpectra(prefix, refantName, sourceName, spw):
    ...
    text_sd = '%12s %6.2f GHz   %6.3f (%.3f) %7.4f (%.4f) %7.4f (%.4f) %7.4f (%.4f)' % (sourceName, np.median(Freq), np.mean(StokesSpec[0]), sdF*np.std(StokesSpec[0]), np.mean(StokesSpec[1]), sdF* np.std(StokesSpec[1]),np.mean(StokesSpec[2]), sdF* np.std(StokesSpec[2]),np.mean(StokesSpec[3]), sdF* np.std(StokesSpec[3]))
    print(text_sd)
