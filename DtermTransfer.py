...
#trkAnt = list(set(antList) - set(scanAnt))
#trkAnt = np.load('%s-REF%s.Ant.npy' % (prefix, refantName)) 
msmd.open(msfile)
...
for spw_index, spw in enumerate(spwList):
    ...
    GA = np.load('%s-SPW%d.GA.npy' % (XYprefix, spw))
    TS = np.load('%s-SPW%d.TS.npy' % (XYprefix, spw))
    ...
    PS = PAVector(np.array(PA), np.ones(len(mjdSec)))
    VisSpec = (VisSpec.transpose(3, 2, 0, 1) / BP_bl).transpose(2,3,1,0)
    GainCaledVisSpec = VisSpec.transpose(1, 2, 0, 3) / (GA[ant0][:,polYindex][:,:,indexList( np.array(mjdSec), TS )]* GA[ant1][:,polXindex][:,:,indexList( np.array(mjdSec), TS )].conjugate()) # GainCaledVisSpec[ch, bl, pol, time]
    #-------- For each scan antenna
    for scanAnt in scanAntList:
        ...
        #
        trcScanVisSpec = CrossPolBL( GainCaledVisSpec[:,trkScnBLMap].transpose(2,0,1,3), trkScnInv)
        ch_index = 32
        Dx, Dy = TransferD(trcScanVisSpec[:,ch_index], DxSpec[:, ch_index], DySpec[:, ch_index], PS)
    #


    #GainCaledVisSpec = VisSpec.transpose(1,0,2,3) / (GA[polYindex][:,ant0]* GA[polXindex][:,ant1].conjugate())

    #chavVis = np.mean(VisSpec[[0,3]][:,chRange], axis=1)
    #GainX, GainY = polariGain(chavVis[0], chavVis[1], QCpUS)

    #Gain = GA[:,:,indexList(TS, np.array(mjdSec))]
    #VisSpec = (((VisSpec.transpose(3, 2, 0, 1) / BP_bl).transpose(3, 1, 2, 0)) / (Gain[ant0][:,polYindex]* Gain[ant1][:,polXindex].conjugate())).transpose(2,0,1,3)
    ...
    '''
        ...
        BPCaledXspec = (Xspec / (BP_ant[ant0]* BP_ant[ant1].conjugate())).transpose(2,3,1,0)

        scanAz, scanEl = AzElMatch(timeStamp, azelTime, AntID, refAntID, AZ, EL)
        scanPA = AzEl2PA(scanAz, scanEl, ALMA_lat) + BandPA
    DxSpec, DySpec = [], []
    for ant_index, antName in enumerate(trkAnt):
        DtermData = np.load('%s-SPW%d-%s.DSpec.npy' % (prefix, spw, antName))
        DxSpec = DxSpec + [DtermData[1] + (0.0 + 1.0j)* DtermData[2]]
        DySpec = DySpec + [DtermData[3] + (0.0 + 1.0j)* DtermData[4]]
    #
