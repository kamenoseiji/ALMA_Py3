...
if 'refIndex' not in locals(): refIndex = list(range(UseAntNum))
if len(refIndex) == 0: refIndex = list(range(UseAntNum))
#-------- Load Aeff file
...
for scan_index in list(range(scanNum)):
    ...
    for spw_index in list(range(spwNum)):
        ...
        #if slopeSNR < 5.0: solution[0], solution[1] = np.percentile(StokesVis[0][visFlag], 70),  0.0
        if slopeSNR < 5.0: solution[0], solution[1] = np.mean(Stokes[0][visFlag]).real,  0.0
        ...
        for pol_index in list(range(4)): text_Stokes[spw_index] = text_Stokes[spw_index] + ' %7.4f (%.4f) ' % (ScanFlux[scan_index, spw_index, pol_index], ErrFlux[scan_index, spw_index, pol_index])
