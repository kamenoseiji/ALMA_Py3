...
for BandName in RXList:
    ...
    checkScan   = QSOscanList[np.argmax(np.array([np.median(abs(abs(scanDic[scan]['UCmQS']) - abs(scanDic[scan]['QCpUS']))* scanDic[scan]['I']* np.sin(scanDic[scan]['EL'] - ELshadow)) for scan in QSOscanList]))]
