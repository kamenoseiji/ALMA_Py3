...
parser.add_option('-c', dest='scanList', metavar='scanList',
    help='Scan List  e.g. 9, 11, 13, 15', default='2')
parser.add_option('-s', dest='spw', metavar='spw',
    help='SPW e.g. 0', default='')
...
'''
prefix = 'NGC_1052'
scanList = [9, 11, 13, 15, 19, 23, 25, 29, 31, 33, 35, 41]
spw = 0
'''
msfile = prefix + '.ms'
...
for scan_index, scan in enumerate(scanList):
    ...
    SNRBL = abs(np.mean(Xspec, axis=(0,1,3)))**2
    Spec = np.mean(np.mean(Xspec, axis=3).dot(SNRBL), axis=0) / np.sum(SNRBL)
    SpecList = SpecList + [Spec]
    WeightList = WeightList + [1.0 / np.var(Spec.imag)]
WeightList = np.array(WeightList)
SpecTmp = np.array(SpecList).T.dot(WeightList)/sum(WeightList)
#-------- Prepare Plots
'''
